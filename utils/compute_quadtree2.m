...
for kx=0:n/2^j_min-1
    for ky=0:n/2^j_min-1
        ...
        [theta,l] = compute_best_direction2(M(selx,sely,:),T,s);
        for i=1:nT
...
for j=j_min+1:j_max
    ...
    for kx=0:n/2^j-1
        for ky=0:n/2^j-1
            ...
            l_sum = L(2*kx+1,2*ky+1,:) + L(2*kx+2,2*ky+1,:) + L(2*kx+1,2*ky+2,:) + L(2*kx+2,2*ky+2,:);
            l_sum = l_sum(:);
